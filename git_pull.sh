...
ScriptsURL=https://github.com/DovFork/jd-scripts
ShellURL=https://github.com/DovFork/jd-base
DockerURL=https://github.com/DovFork/jd-docker
...
function Npm_InstallSub {
  if [ -n "${isTermux}" ]
  then
    npm install --no-bin-links || npm install --no-bin-links --registry=https://registry.npmjs.org
  elif ! type yarn >/dev/null 2>&1
  then
    npm install || npm install --registry=https://registry.npmjs.org
  else
    echo -e "检测到本机安装了 yarn，使用 yarn 替代 npm...\n"
    yarn install || yarn install --registry=https://registry.npmjs.org
  fi
}
